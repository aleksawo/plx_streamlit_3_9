...
def run_ankerkrefter_2D(s_o, g_o, s_i, g_i):
    ...
    with col2:
        ...
        if 'phase_data' not in st.session_state.keys():
            ...
            st.session_state['dummy_data'] = anker_data
        else:
            anker_data = st.session_state['dummy_data']
        checkbox_container(anker_data)
        x_kord, y_kord, type_anker = split_koordinates(get_selected_checkboxes(), g_i)
